...
the `c()` function. For example, we can create a vector of check-in IDs from our dataset:
...
3. If you have a numeric vector and need to convert it to character, what function would you use?
...
3. To explicitly convert a numeric vector to character:
```{r}
numbers <- c(1, 2, 3)
...
dates_as_strings <- c("2020-02-28", "2020-03-01")

# Attempt to calculate the difference by converting strings to numeric days
days_difference <- as.numeric(substr(dates_as_strings[2], 9, 10)) - as.numeric(substr(dates_as_strings[1], 9, 10))
days_difference # Incorrect computation: Ignores months and leap years
```
...
dates <- as.Date(c("2020-02-28", "2020-03-01"))
days_difference <- dates[2] - dates[1]
days_difference # Correctly computes 2 days, accounting for February 29 in the leap year
```
