...
```{r, results="show", purl=FALSE}
checkin_lengths <- c(64, 74, 36, 287)
checkin_lengths[c(TRUE, FALSE, TRUE, TRUE)]
```
...
```{r, results="show", purl=FALSE}
checkin_lengths > 60    # will return logicals with TRUE for the indices that meet the condition
## so we can use this to select only the values greater than 2866
checkin_lengths[checkin_lengths > 60]
```
...
```{r, results="show", purl=FALSE}
checkin_lengths[checkin_lengths < 60 | checkin_lengths > 80]
checkin_lengths[checkin_lengths >= 60 & checkin_lengths <= 80]
```
...
# 3.
checkin_lengths_above_55 <- rooms_no_na[checkin_lengths_no_na > 55]
