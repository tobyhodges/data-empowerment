...
1. As character strings (e.g., "2023-11-15", "11/15/2023")
2. As Date objects which are a special data type

When dates are stored as strings, they're treated like any other text:

```{r}
dates_as_strings <- c("2023-01-15", "2023-02-20", "2023-03-25")
typeof(dates_as_strings)
```

However, storing dates as proper Date objects offers several advantages:
...
To convert strings to Date objects, use the `as.Date()` function:

```{r}
# Convert strings to Date objects
dates <- as.Date(dates_as_strings)
typeof(dates)
class(dates)
```

The default format is "YYYY-MM-DD" (ISO standard). If your dates are in a different format, you need to specify it:

```{r}
# American-style dates (month/day/year)
us_dates <- c("1/15/2023", "2/20/2023", "3/25/2023")
as.Date(us_dates, format = "%m/%d/%Y")
```

Once you have Date objects, you can:

```{r}
# Calculate intervals between dates
date1 <- as.Date("2023-01-15")
date2 <- as.Date("2023-03-25")
date2 - date1  # Returns the difference in days

# Extract parts of dates
format(date1, "%m")  # Extract month
format(date1, "%Y")  # Extract year
format(date1, "%d")  # Extract day

# Sort dates
random_dates <- as.Date(c("2023-03-25", "2022-12-01", "2023-01-15"))
sort(random_dates)
```

For more complex date-time operations, consider the `lubridate` package, which makes working with dates even easier:

```{r, eval=FALSE}
# library(lubridate)
# ymd("2023-01-15")  # Year-Month-Day format
# mdy("01/15/2023")  # Month-Day-Year format
# dmy("15/01/2023")  # Day-Month-Year format
```
...
1. Create a vector of three dates as strings: "2022-10-15", "2023-01-20", "2022-07-31"
2. Convert these strings to Date objects
3. Sort the dates from earliest to latest
4. Calculate how many days passed between the earliest and latest date
...
```{r}
# 1. Create a vector of dates as strings
date_strings <- c("2022-10-15", "2023-01-20", "2022-07-31")

# 2. Convert to Date objects
my_dates <- as.Date(date_strings)

# 3. Sort the dates
sorted_dates <- sort(my_dates)
sorted_dates

# 4. Calculate days between earliest and latest
latest_date <- max(my_dates)
earliest_date <- min(my_dates)
days_difference <- latest_date - earliest_date
days_difference
```
