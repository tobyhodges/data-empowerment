...
# No need to download data as we'll use the provided anonymized check-in data
```
...
```
```{r, purl=FALSE}
xyplot(checkin_hour ~ device | location, data = checkins,
...
To add a geom to the plot use the `+` operator. Let's create a scatter plot of check-in hours vs. precincts:
...
```{r facet-by-device, fig.alt="Multi-panel line plot showing check-ins by hour for each device."}
# Create hourly counts by device
hourly_by_device <- checkins %>%
  count(checkin_hour, device)
...
```{r facet-by-location-device, fig.alt="Multi-panel point plot showing check-in patterns by hour for each location and device combination."}
# Count check-ins by hour, location, and device
hourly_by_loc_device <- checkins %>%
