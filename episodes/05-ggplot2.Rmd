...
data <- read_csv(here("episodes", "data", "anonymized_data_2_sample.csv"))
```
...
stored in a data frame simpler. It provides a programmatic interface for
...
1. use the `ggplot()` function and bind the plot to a specific data frame.
...
column to our hourly_counts data frame that indicates whether the count at that 
...
dots on the plot than there rows in our data frame. This should lead us to believe 
...
create a new data frame (`prop_device`) with a new column named "prop", 
...
Now, we can use this new data frame to create our plot showing the *proportion* 
...
westeros_map <- st_read(here("episodes", "data", "polygons_example_GoT.geojson"), quiet = TRUE)
...
function. This function allows for two data frames to be linked based off of a
specified variable (in our case, the "id"):
```{r load-voting-data, eval=TRUE}
#read in data and save to object
got_votes <- read_csv(here("episodes", "data", "voting_example_GoT.csv"))
...
```{r map-solution}
#read in data
checkin_data <- read_csv(here("episodes", "data", "avg_checkins.csv"))
...
the text, title, subtitle, and grids. You can find a full list either by following 
[this link](https://ggplot2.tidyverse.org/reference/theme.html) or using the panel 
...
ggsave("data/device_prop.png", plot, width = 10, height = 6, dpi = 300)
```
