...
checkins_sample_wide <- read_csv("data/anonymized_data_2_sample.csv")
```
...
# Count number of check-ins by hour and device
hourly_by_device <- checkins_sample_wide %>%
  count(hour, device)

# Plot using faceting
ggplot(hourly_by_device, aes(x = hour, y = n)) +
  geom_line(group = 1) +  # One line per device; group = 1 if only one device
  geom_point() +
  labs(title = "Hourly Check-ins by Device",
       x = "Hour of Day",
       y = "Number of Check-ins") +
  facet_wrap(~ device, scales = "free_y") +  # Will show one facet per device
  theme_bw()
```
