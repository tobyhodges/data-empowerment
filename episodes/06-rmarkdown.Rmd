...
applied to text within your file and converted into formatted elements
...
This report uses the **tidyverse** package along with the *Anonymized* dataset, 
...
However, it is no longer enough to load **tidyverse** from the console
-- when working with R Markdown, you much ensure any necessary packages
...
To load **tidyverse** and our `anonymized_data.csv` file, we will insert
...
this by creating a new code chunk and calling it 'anon-tbl'. Or, you can
come up with something more creative (just remember to stick to the
naming rules).

It isn't necessary to **Knit** your document every time you want to see
the output. Instead you can run the code chunk with the green triangle
in the top right corner of the the chunk, or with the keyboard
...
the code and output from printing in the knitted document. There are
additional options available to customize how the code-chunks are
...
the table, and re-**Knit** to see what each option does to the output.
...
-   `eval = FALSE` and `echo = FALSE` will neither run the code in the
    chunk, nor show the code in the knitted document. The code chunk
    essentially doesn't exist in the knitted document!
-   `include = FALSE` will not display the code nor the output, but it
    *will* be ran, with the output stored for later use!
:::
::::
...
is the number of back-ticks. In-line R code uses one back-tick
...
The code will display today's date in the output document (or,
technically, the date the document was last knitted).

The best way to use in-line R code is by preparing the output in code
chunks, minimizing the code needed to produce the output. Let's say
we're interested in presenting the total check-ins for a specific
precinct.

```{r in-line-code, purl=FALSE}
#create a summary data frame with the total check-ins per precinct
df <- data %>%
      group_by(precinct) %>%
      summarize(total_checkins = n())
...
Finally, we will be including a plot, so our document is a little more
colorful and a little less boring to read!
...
We recommend one of the colorful plots! :)
...
To convert an R Markdown file to a PDF, Word Document, or other option,
you can either click the little triangle next to the **Knit** button to
get a drop-down menu or put `pdf_document` or `word_document` in the
...
The toolbar is accessible by using the settings drop-down menu (next to
the 'Knit' drop-down menu) to select 'Use Visual Editor', also
...
Additionally, you can obtain citation information from relevant packages by using 
`citation("package")`.
:::
