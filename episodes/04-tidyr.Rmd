...
To make sure everyone will use the same datasets for this lesson, we'll be
reading in the updated version of the Anonymized Dataset (as created in 
"Starting With Data"), as well as the Check-In Dataset (which we will cover at
...
Next, we will read in the Anonymized Data:
```{r, eval=TRUE, message=FALSE, purl=FALSE}
anon_data <- read_csv(
                here("data", "anonymized_data_2.csv"),
             )
```
...
First, we will explore qualities of the `anon_data` data and how they relate 
to these different types of data formats.

### Long and Wide Data Formats

In `anon_data`, each row contains the values of variables associated
...
If you were to look at the entire `anon_data` dataset, you would notice that the 
...
![Animation showing pivot_wider and pivot_longer functions transforming data between long and wide formats](fig/tidyr-pivot_wider_longer.gif){alt="A gif showing how long and wide dataframe layouts relate to each other."}
...
In `anon_data`, each row contains values associated with each
...
Let's use `pivot_wider()` to transform `anon_data` to create new columns for each
hour represented within the data.

To help with understanding, we will be walking through the transformation line-by-line. 

First we create a new object (`anon_data_tc`) based on the `anon_data` 
tibble:
```{r, eval=FALSE}
anon_data_tc <- anon_data %>%
```
...
look at the resulting `anon_data_tc` tibble to gain a better understanding.
...
location within the dataset. We can verify this by counting the number of unique 
location values within `anon_data`:
```{r, purl=FALSE}
n_distinct(anon_data$location)
```
...
```{r, purl=FALSE}
anon_data_tc %>%
  mutate(total_checkins = rowSums(anon_data_tc[-1])) %>%
...
We created `anon_data_tc` by reshaping the data. Replicate this process
to create a tibble named `anon_data_total` that shows the total number of 
...
latest similarly to the `anon_data_tc` tibble. There should only be one 
...
anon_data_total_2 <- anon_data_total_2 %>%
  mutate(total_checkins = rowSums(anon_data_total_2))
...
The opposing situation could occur if we had been provided with the `anon_data_tc`
...
```{r, purl=FALSE}
anon_data_tc_long <- anon_data_tc %>%
...
In the last exercise, you created the wide tibble, `anon_data_total`. In this 
exercise, your goal is to reverse this transformation using `pivot_longer()`.

Create a tibble called `anon_data_total_long` that has two columns: one for the 
...
```{r}
anon_data_total_long <- anon_data_total %>%
...
The Check-In Dataset is an example of a "messy" dataset that tracks when 
people check-in to a voting location! In the context of the dataset, labels 
...
"normal". Within this dataset, missing data is encoded as "NULL".
...
R to automatically convert all the "NULL" entries in the dataset into `NA`.

Below, we will be reading in the Check-In Dataset using the additional line:
```{r, eval=TRUE, message=FALSE, purl=FALSE}
checkin_data <- read_csv(
                   here("episodes", "data", "checkin_data.csv"),
                   na = "NULL"
                )
```
...
At first glance, we can see this dataset is wide, with each label tacked onto 
...
and a `label` column. Let's apply this pivot to a new tibble, named `checkin_data_clean`, 
below:
```{r, eval=TRUE, message=FALSE, purl=FALSE}
checkin_data_clean <- checkin_data %>%
  pivot_longer(cols = everything(),
               names_to = "label",
               values_to = "duration")

head(checkin_data_clean)
```

Oh no! That's a lot of `NA` values. Taking a closer look at the original data, 
we can see the first value within the dataset consists of a duration of `80` 
underneath the `"CheckIn_Duration_"` column. Looking at our in-progress, "clean" 
dataset, we can see the labels that do not apply to this duration are listed as 
`NA`. 

Since the labels that have a duration of `NA` do not matter within our data set, 
we can drop them from the tibble completely:
```{r, eval=TRUE, message=FALSE, purl=FALSE}
checkin_data_clean <- checkin_data_clean %>%
  drop_na()
...
```{r, eval=TRUE, message=FALSE, purl=FALSE}
checkin_data_clean_2 <- checkin_data %>%
...
head(checkin_data_clean_2)
```

Since our data has been cleaned, we can now export it as `checkin_clean.csv` for 
...
data folder:
```{r, eval=TRUE, message=FALSE, purl=FALSE}
write_csv(checkin_data_clean, "data/checkin_clean.csv")
```
