...
- Describe the roles of variable names and their associated values when a table 
  is reshaped.
- Reshape a data frame from long to wide format and back with the `pivot_wider` 
...
```{r, eval=TRUE, message=FALSE, purl=FALSE}
anon_data <- read_csv(here("data", "anonymized_data_2.csv"))
```
...
format, where each observation occupies only one row in the data frame.
...
If you were to look at the entire `anon_data` data set, you would notice that the 
...
The Check-In Dataset is an example of a "messy" data set that tracks when 
